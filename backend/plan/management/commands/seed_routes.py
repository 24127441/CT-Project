...
class Command(BaseCommand):
    help = 'Nạp dữ liệu mẫu cho bảng Route'
    ...
    def handle(self, *args, **kwargs):
        ...
        try:
            ...
            self.stdout.write(self.style.WARNING("--- Đã xóa toàn bộ dữ liệu Route cũ ---"))
            ...
            for data in routes_data:
                ...
                Route.objects.create(
                    ...
                    # FIX: Thêm trường gallery để không bị lỗi NotNull trong DB và khớp với models.py
                    gallery=[],
                )
                count += 1

            self.stdout.write(self.style.SUCCESS(f"✅ Đã nạp thành công {count} cung đường trekking."))

        except Exception as e:
            self.stdout.write(self.style.ERROR(f"❌ Lỗi khi chạy seed: {e}"))